...
class RefineRequest(BaseModel):
    session_id: str
    action: str = Field(..., example="reject", description="'reject' or 'show'")
    value: str = Field(..., example="iPhone 9", description="Product title to reject or new search query")
...
def refine_session(request: RefineRequest):
    """Refines an existing shopping session by rejecting an item or searching for more."""
    ...
    if request.action == "reject":
        ...
        raise HTTPException(status_code=400, detail="Invalid action. Use 'reject' or 'show'.")
